...
if (!function_exists('getFrontMenuItems')) {
    /**
     * Converts items (json string) to array and return array.
     */
    function getFrontMenuItems($type = 'frontend', $id = null)
    {
        $menu = new \App\Models\Menu\Menu();
        $menu = $menu->where('type', $type);
        if (!empty($id)) {
...
if (!function_exists('renderFrontMenuItems')) {
    /**
     * render sidebar menu items after permission check.
     */
    function renderFrontMenuItems($items, $viewName = 'frontend.includes.partials.sidebar-item')
    {
        foreach ($items as $item) {
            // if(!empty($item->url) && !Route::has($item->url)) {
            //     return;

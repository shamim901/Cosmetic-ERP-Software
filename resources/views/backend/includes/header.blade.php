...
{{--           {{ substr(app_name(), 0, 1) }}--}}
            <img height="100%" width="100%" class="navbar-brand" src="{{asset('storage/img/favicon/'. $settings->favicon)}}">
        </span>

        <!-- logo for regular state and mobile devices -->
        <span class="logo-lg">
            @if($settings->logo)
                <img height="100%" width="100%" class="navbar-brand" src="{{asset('storage/img/logo/'. $settings->logo)}}">
            @else
                {{ app_name() }}
             @endif
        </span>
